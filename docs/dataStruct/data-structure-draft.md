...
ディスク上のシナリオ情報中の70ブロック目の内容は, 以下のようになっており, 上記の式で算出された位置に, HELMの情報が格納されることが確認できる。
...
AppleIIでの各ピクセルの発行色は, 画面上でのピクセルのX座標に応じて決定される。AppleIIで, 白色や黒色を表示するためには, X座標上で連続する2つのピクセルを点灯または消灯させる必要がある( [AppleIIビデオメモリにおけるピクセルの6色表示制御](#AppleIIビデオメモリにおけるピクセルの6色表示制御) 参照)。
...
発光色の選択は, 以下のように決定される。
  ...
  1. ビデオメモリ上のピクセルのX座標が偶数の場合,
     ...
     a. ビデオメモリ上のピクセルのX座標が偶数の場合, かつ, カラー選択ビットが立っていたら青色を発光する。

     b. ビデオメモリ上のピクセルのX座標が偶数の場合, かつ, カラー選択ビットが立っていたら紫色を発光する。
